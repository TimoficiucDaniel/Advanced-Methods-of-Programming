...
public class Scene2Controller {
    ...
    @FXML private TableView locktable;
    @FXML private TableColumn<Pair<Integer,Integer>,Integer> lockAddr;
    @FXML private TableColumn<Pair<Integer,Integer>,Integer> lockVal;
    ...
    public void initScene()
    {
        pr = new PrgState(new MyDictionary<>(new HashMap<>()), new MyStack<>(new Stack<>()), new MyList<>(new ArrayList<>()), stmt,
                new MyDictionary<>(new HashMap<>()), new Heap<>(new HashMap<>()), 1, new Lock<>(new HashMap<>()));
        ArrayList<PrgState> list = new ArrayList<>();
        list.add(pr);
        repo = new Repo(pr,"GuiLog");
        ctrl = new Controller(repo);
        lockAddr.setCellValueFactory(new PropertyValueFactory<Pair<Integer,Integer>,Integer>("first"));
        lockVal.setCellValueFactory(new PropertyValueFactory<Pair<Integer,Integer>,Integer>("second"));
    ...
    private void populateLockTable(){
        ...
        if (ctrl.getnrOfPrg() > 0)
        {
            ...
            for (Map.Entry<Integer, Integer> r : ctrl.getCrtPrg().get(0).getLocktable().getSet()) {
