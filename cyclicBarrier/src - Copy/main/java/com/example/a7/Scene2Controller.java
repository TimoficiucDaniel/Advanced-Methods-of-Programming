...
public class Scene2Controller {
    ...
    @FXML private TableView barriertable;
    @FXML private TableColumn<ThreePair<Integer,Integer,List<Integer>>,Integer> index;
    @FXML private TableColumn<ThreePair<Integer,Integer,List<Integer>>,Integer> number;
    @FXML private TableColumn<ThreePair<Integer,Integer,List<Integer>>,List<Integer>> lists;
    ...
    public void initScene()
    {
        ...
        pr = new PrgState( new MyDictionary<>(new HashMap<>()),new MyStack<>(new Stack<>()), new MyList<>(new ArrayList<>()),stmt,
                new MyDictionary<>(new HashMap<>()), new Heap<>(new HashMap<>()), 1,barr);
        ArrayList<PrgState> list = new ArrayList<>();
        list.add(pr);
        repo = new Repo(pr,"GuiLog");
        ctrl = new Controller(repo);
        index.setCellValueFactory(new PropertyValueFactory<ThreePair<Integer,Integer,List<Integer>>,Integer>("first"));
        number.setCellValueFactory(new PropertyValueFactory<ThreePair<Integer,Integer,List<Integer>>,Integer>("second"));
        lists.setCellValueFactory(new PropertyValueFactory<ThreePair<Integer,Integer,List<Integer>>,List<Integer>>("third"));
    ...
    private void populateBarrierTable() {
        if (ctrl.getnrOfPrg() > 0) {
            ...
            for (Map.Entry<Integer, Pair<Integer, List<Integer>>> r : ctrl.getCrtPrg().get(0).getBarrier().getSet()) {
                ThreePair<Integer,Integer,List<Integer>> pair = new ThreePair<>(r.getKey(), r.getValue().first, r.getValue().second);
                obsList.add(pair);
            }
