...
public class Scene2Controller {
    ...
    @FXML private TableView latchtable;
    @FXML private TableColumn<Pair<Integer,Integer>,Integer> address;
    @FXML private TableColumn<Pair<Integer,Integer>,Integer> count;
    ...
    public void initScene()
    {
        ...
        pr = new PrgState( new MyDictionary<>(new HashMap<>()),new MyStack<>(new Stack<>()), new MyList<>(new ArrayList<>()),stmt,
                new MyDictionary<>(new HashMap<>()), new Heap<>(new HashMap<>()), 1,new Latch<>(new HashMap<>()));
        ArrayList<PrgState> list = new ArrayList<>();
        list.add(pr);
        repo = new Repo(pr,"GuiLog");
        ctrl = new Controller(repo);
        address.setCellValueFactory(new PropertyValueFactory<Pair<Integer,Integer>,Integer>("first"));
        count.setCellValueFactory(new PropertyValueFactory<Pair<Integer,Integer>,Integer>("second"));
    ...
    private void populateLatch() {
        ...
        if (ctrl.getnrOfPrg() > 0)
        {
            ...
            for (Map.Entry<Integer, Integer> r : ctrl.getCrtPrg().get(0).getLatch().getSet()) {
